...
void Student::splitLine(const std::string& line) {
    ...
    /*Array Reference : userArray[0] store keyvalue, schoolArray[1]-[4] store Student Variable*/
...
void Student::editStudent() {
    ...
    if (it != studentMapPtr->end())
    {
        ...
        if (!outFile) {
            ...
            // Close the file
            outFile.close();
            std::cout << newStudentObj.firstName << " " << newStudentObj.lastName << " has been added successfully!\n";
        }
    }
    else
...
void Student::updateStudentFile(const std::map<short, Student>::iterator& _studentIt) {
    ...
    if (!outFile) {
        ...
        outFile << _studentIt->first << "-" + _studentIt->second.firstName << "-" << _studentIt->second.lastName << "-" <<
            _studentIt->second.dateOfBirth << "-" << _studentIt->second.gender << std::endl;
