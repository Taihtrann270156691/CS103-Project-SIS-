...
void Teacher::displayInfo() const {
    if (teacherMapPtr->size() != 0) { 
        for (auto it = teacherMapPtr->begin(); it != teacherMapPtr->end(); it++) {
            ...
            std::cout << "Role: " << it->second.role << std::endl << std::endl;
        }
    }
    else
...
void Teacher::readTeacherFile() {
    ...
    if (fileExist.is_open()) { //file exist
        ...
        std::cout << "Error cannot open Teacher.txt file." << std::endl;
...
void Teacher::editTeacher() {
    ...
    if (teacherMapPtr->find(finalKey) != teacherMapPtr->end())
    {
        Teacher newTeacherObj = Teacher();
        userInput(newTeacherObj);
        teacherMapPtr->find(finalKey)->second.firstName = newTeacherObj.firstName;
        teacherMapPtr->find(finalKey)->second.lastName = newTeacherObj.lastName;
        teacherMapPtr->find(finalKey)->second.dateOfBirth = newTeacherObj.dateOfBirth;
        teacherMapPtr->find(finalKey)->second.gender = newTeacherObj.gender;
        teacherMapPtr->find(finalKey)->second.role = newTeacherObj.role;
        
        // Open the file in append mode
        std::ofstream outFile(teacherFile, std::ios::app);

        if (!outFile) {
            std::cerr << "Error opening Teacher.txt file!" << std::endl;
        }
        else {
...
void Teacher::deleteTeacher() {
    ...
    if (it != teacherMapPtr->end())
    {
        ...
        if (file.is_open()) {
            ...
            std::cout << "Error opening Teacher.txt file." << std::endl;
    }
    else
