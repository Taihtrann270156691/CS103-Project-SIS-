...
short UserLogin::UserMenu(bool& _admin){
	...
	if (_admin) {
		while (true) {
			...
			std::cout << "4 - to display list of users\n";
...
void UserLogin::readUserFile() {
	...
	if (fileExist.is_open()) { //file exist
		...
		while (std::getline(fileExist, line)) { // Read each line
			if (!line.empty()) //making sure it's not an empty string
				splitLine(line);
		}
...
/// <summary>
/// split raw data into a readable chunk of data and fetch them into user map container
/// </summary>
...
void UserLogin::addNewUser(){
	...
	auto it = --myUserMap->end(); //fetch the last element
	short newKey = it->first + 1; //fetch the last element's key
...
void UserLogin::display() const {
	...
	for (auto it = myUserMap->begin(); it != myUserMap->end(); it++) {
		std::cout << "User name: " << it->second.userName << ", Password: " << it->second.password << std::endl;
	}
}