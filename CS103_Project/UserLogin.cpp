...
/// <returns></returns>
bool UserLogin::userLogin(bool& _admin){
	UserLogin userObj;
	bool login = false;
	short tryCount = 3;
	short key;
...
short UserLogin::adminMenu(){
	std::string choice;
	std::cout << "Enter 1 - to add new user, 2 - to delete, 3 - to change user password, or -1 to go back\n";
...
/// </summary>
void UserLogin::editUserPassword(){
	std::string key;
	std::cout << "Please provide key value to select a user." << std::endl;
	while (true) {
		std::cout << "Please enter key: ";
		std::getline(std::cin, key);
		//lambda function to check for positive number
